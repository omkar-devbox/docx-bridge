        ...
        if isinstance(headers, list):
            for hdr in headers:
                if isinstance(hdr, dict):
                    hdr_type = hdr.get("type", "default")
                    rel_id = hdr.get("relationshipId") or hdr.get("id") or hdr.get("rId") or ""
                    if rel_id:
                        ET.SubElement(
                            sect_pr,
                            qn("w:headerReference"),
                            {
                                qn("w:type"): str(hdr_type),
                                qn("r:id"): str(rel_id),
                            },
                        )
        ...
        if isinstance(footers, list):
            for ftr in footers:
                if isinstance(ftr, dict):
                    ftr_type = ftr.get("type", "default")
                    rel_id = ftr.get("relationshipId") or ftr.get("id") or ftr.get("rId") or ""
                    if rel_id:
                        ET.SubElement(
                            sect_pr,
                            qn("w:footerReference"),
                            {
                                qn("w:type"): str(ftr_type),
                                qn("r:id"): str(rel_id),
                            },
                        )
