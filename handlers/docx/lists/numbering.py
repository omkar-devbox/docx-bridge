...
from handlers.docx.text import RunHandler
...
class NumberingHandler(BaseHandler):
    ...
    def _parse_number_instance(
        ...
        for l_ov in num_element.findall(qn("w:lvlOverride")):
            ilvl_val = l_ov.attrib.get(qn("w:ilvl"), "0")
            ilvl_int = int(ilvl_val) if ilvl_val.isdigit() else ilvl_val
            ov_data: dict[str, Any] = {
                "level": ilvl_int,
                "ilvl": ilvl_int,
            }
            start_ov = l_ov.find(qn("w:startOverride"))
            if start_ov is not None:
                s_val = start_ov.attrib.get(qn("w:val"), "")
                ov_data["startOverride"] = int(s_val) if s_val.isdigit() else s_val
            lvl_overrides.append(ov_data)

        if lvl_overrides:
            res["lvlOverride"] = lvl_overrides
            res["levelOverrides"] = lvl_overrides
    ...
    def to_json(
        ...
        **kwargs,
        ...
        return {
            ...
            "numbering": numbering,
            "abstractNumbers": abstract_numbering,
            "instances": numbering,
        }
    ...
    def _build_number_instance(
        ...
        num_id = item.get("numId") if "numId" in item else item.get("id", "")
        num = ET.Element(
            qn("w:num"),
            {
                qn("w:numId"): str(num_id),
            },
        )
        ...
        lvl_overrides = (
            item.get("lvlOverride")
            or item.get("lvlOverrides")
            or item.get("levelOverrides")
            or []
        )
        if isinstance(lvl_overrides, dict):
            lvl_overrides = [lvl_overrides]
        for ov in lvl_overrides:
            if not isinstance(ov, dict):
                continue
            ilvl = str(ov.get("ilvl") if "ilvl" in ov else ov.get("level", "0"))
            ov_el = ET.SubElement(num, qn("w:lvlOverride"), {qn("w:ilvl"): ilvl})
            if "startOverride" in ov:
                ET.SubElement(ov_el, qn("w:startOverride"), {qn("w:val"): str(ov["startOverride"])})
            if "lvl" in ov and isinstance(ov["lvl"], dict):
                lvl_dict = dict(ov["lvl"])
                lvl_dict.setdefault("level", ilvl)
                ov_el.append(self._build_level(lvl_dict))
    ...
    def to_xml(
        ...
        abstract_numbering = (
            data.get("abstractNumbering")
            or data.get("abstractNumbers")
            or data.get("abstract_numbering")
            or []
        )
