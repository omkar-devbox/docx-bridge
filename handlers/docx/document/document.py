    ...
    def to_xml(
        ...
        if isinstance(data, list):
            ...
            if isinstance(raw_sections, list):
                sections_data = [dict(s) for s in raw_sections]
                if "content" in data and isinstance(data["content"], list):
                    has_sec_content = any("content" in sec for sec in sections_data)
                    if not has_sec_content and sections_data:
                        sections_data[0]["content"] = data["content"]
