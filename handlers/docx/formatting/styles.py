...
class StylesHandler(BaseHandler):
    ...
    def to_json(
        ...
        for style_element in element.findall(qn("w:style")):
            ...
            istd_val = style_element.attrib.get(qn("w:istd"))
            if istd_val is not None:
                try:
                    entry["istd"] = int(istd_val)
                except ValueError:
                    entry["istd"] = istd_val
            ...
            if run_properties is not None:
                ...
                clean_r = {
                    k: v for k, v in run_json.items()
                    if k not in ("text", "type") and v not in (None, "", {})
                }
                if clean_r:
                    entry["run"] = clean_r
            ...
            if paragraph_properties is not None:
                ...
                p_dummy = ET.Element(qn("w:p"))
                p_dummy.append(paragraph_properties)
                paragraph_json = paragraph_handler.to_json(
                    p_dummy,
                    simple=True,
                )
                ...
                clean_p = {
                    k: v for k, v in paragraph_json.items()
                    if k not in ("text", "type", "runs", "style") and v not in (None, "", {})
                }
                if clean_p:
                    entry["paragraph"] = clean_p
    ...
    def to_xml(
        ...
        for style_id, style_data in styles_list:
            ...
            if style_data.get("istd") is not None:
                style_attributes[qn("w:istd")] = str(style_data["istd"])
            ...
            if style_data.get("uiPriority") is not None:
                ET.SubElement(
                    style_element,
                    qn("w:uiPriority"),
                    {
            ...
            if style_type != "character":
                p_data = dict(style_data)
                if isinstance(style_data.get("paragraph"), dict):
                    p_data.update(style_data["paragraph"])
            ...
            r_data = dict(style_data)
            if isinstance(style_data.get("run"), dict):
                r_data.update(style_data["run"])
