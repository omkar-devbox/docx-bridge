"""Handler for WordprocessingML endnotes (word/endnotes.xml)."""
...
class EndnotesHandler(BaseHandler):
    """Handles parsing and generating word/endnotes.xml."""
    ...
    def to_json(
        ...
        """Parse endnotes XML into a list of endnote dictionaries."""
        endnotes: list[dict[str, Any]] = []

        for note_el in element.findall(qn("w:endnote")):
    ...
    def to_xml(
        self,
        endnotes_data: list[dict[str, Any]] | dict[str, Any],
    ) -> ET.Element:
        """Construct endnotes.xml element tree."""
        root = ET.Element(qn("w:endnotes"))
        notes_list = endnotes_data if isinstance(endnotes_data, list) else endnotes_data.get("endnotes", [])
        ...
        if not has_sep:
            sep = ET.SubElement(root, qn("w:endnote"), {qn("w:type"): "separator", qn("w:id"): "-1"})
            p_sep = ET.SubElement(sep, qn("w:p"))
            r_sep = ET.SubElement(p_sep, qn("w:r"))
            ET.SubElement(r_sep, qn("w:separator"))

            cont_sep = ET.SubElement(root, qn("w:endnote"), {qn("w:type"): "continuationSeparator", qn("w:id"): "0"})
        ...
        for note in notes_list:
            ...
            note_el = ET.SubElement(root, qn("w:endnote"), attrs)
