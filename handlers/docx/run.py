...
class RunHandler(BaseHandler):
    ...
    def to_json(
        ...
        if has_last_rendered_page_break:
            result["lastRenderedPageBreak"] = True

        # Footnote reference
        fn_ref = element.find(qn("w:footnoteReference"))
        if fn_ref is not None:
            raw_id = fn_ref.attrib.get(qn("w:id"), "")
            result["footnoteReference"] = int(raw_id) if raw_id.isdigit() else raw_id

        # Endnote reference
        en_ref = element.find(qn("w:endnoteReference"))
        if en_ref is not None:
            raw_id = en_ref.attrib.get(qn("w:id"), "")
            result["endnoteReference"] = int(raw_id) if raw_id.isdigit() else raw_id

        # Comment reference
        cm_ref = element.find(qn("w:commentReference"))
        if cm_ref is not None:
            raw_id = cm_ref.attrib.get(qn("w:id"), "")
            result["commentReference"] = int(raw_id) if raw_id.isdigit() else raw_id

        # Field character
        fld_char = element.find(qn("w:fldChar"))
        if fld_char is not None:
            result["fldChar"] = fld_char.attrib.get(qn("w:fldCharType"), "")

        # Field instruction text
        instr_el = element.find(qn("w:instrText"))
        if instr_el is not None and instr_el.text:
            result["instruction"] = instr_el.text.strip()
    ...
    def _build_run_properties(
        ...
        if font_size is not None:
            if isinstance(font_size, float):
                font_size = int(round(font_size * 2))
            elif isinstance(font_size, str) and font_size.endswith("pt"):
                try:
                    font_size = int(round(float(font_size[:-2]) * 2))
                except ValueError:
                    pass
            elif isinstance(font_size, (int, str)):
                try:
    ...
    def to_xml(
        ...
        if text:
            cleaned_text = INVALID_XML_CHARS_RE.sub("", str(text))
            if cleaned_text:
        ...
        # Last rendered page break
        if data.get("lastRenderedPageBreak"):
            ET.SubElement(run, qn("w:lastRenderedPageBreak"))

        # Footnote reference
        if "footnoteReference" in data or "footnote" in data:
            fn_id = str(data.get("footnoteReference", data.get("footnote")))
            ET.SubElement(run, qn("w:footnoteReference"), {qn("w:id"): fn_id})

        # Endnote reference
        if "endnoteReference" in data or "endnote" in data:
            en_id = str(data.get("endnoteReference", data.get("endnote")))
            ET.SubElement(run, qn("w:endnoteReference"), {qn("w:id"): en_id})

        # Comment reference
        if "commentReference" in data:
            cm_id = str(data["commentReference"])
            ET.SubElement(run, qn("w:commentReference"), {qn("w:id"): cm_id})

        # Field character
        if "fldChar" in data:
            ET.SubElement(run, qn("w:fldChar"), {qn("w:fldCharType"): str(data["fldChar"])})

        # Field instruction
        if "instruction" in data:
            instr_el = ET.SubElement(run, qn("w:instrText"))
            instr_el.text = str(data["instruction"])
